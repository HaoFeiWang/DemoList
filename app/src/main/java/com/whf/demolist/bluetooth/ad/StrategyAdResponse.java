...
public class StrategyAdResponse implements StrategyAd {

    private static final String TAG = "BLE_TEST_" + StrategyAdResponse.class.getSimpleName();
    private static final int ADVERTISING_TIME = 60 * 1000;

    private static StrategyAdResponse instance;
    ...
    private StrategyAdResponse() {
        sourceAdData = new ArrayMap<>();
        achieveAdData = new ArrayMap<>();
    }

    public synchronized static StrategyAdResponse getInstance() {
        if (instance == null) {
            instance = new StrategyAdResponse();
        }
        return instance;
    }
